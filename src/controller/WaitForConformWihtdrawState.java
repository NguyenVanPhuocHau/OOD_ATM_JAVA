...
public class WaitForConformWihtdrawState extends AtmState {

	public WaitForConformWihtdrawState(AtmMachine atm) {
		super(atm);
		...
		Runnable setTextRun = new Runnable() {
			public void run() {
				try {
					...
					atm.getScreen().takeCardBeforeCashDisplay();
					...
					atm.getConnect().setAtmState(new WaitForTakeCardBeforeCashAndBillState(atm));
		...
		Runnable setTextRun = new Runnable() {
			public void run() {
				try {
					...
					atm.getTakeAndReceivePart().getCardReader().getjTextCardNumber().setEnabled(true);
					atm.getTakeAndReceivePart().setHasOrNotCard("Lấy thẻ ra");
					atm.getConnect().setAtmState(new WaitForTakeCardBeforeCashNotBillState(atm));
